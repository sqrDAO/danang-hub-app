...
## Acceptance
- [ ] `/member/bookings` renders the page title, then HubClosureNotice, then the amenity list.
- [ ] `/admin/bookings` renders the page title, then HubClosureNotice beside the header actions, then the filters.
- [ ] `/member/bookings` does not leave the 4rem `.page-header` gap between the title and the banner.
- [ ] After visiting `/member` then `/member/bookings` or `/admin/bookings`, title-to-banner gap stays the header `gap` (1rem), not 4rem.
- [ ] NOT: do not restyle the notice or change its copy.
- [ ] NOT: do not add or remove the notice on any other page.
