...
const formatAvailableDays = (days, t) => {
...
const uploadPendingPhotos = async (amenityId, pendingFiles, photos) => {
...
const AmenityCardPhoto = ({ amenity, t, onView }) => {
  if (!amenity.photos || amenity.photos.length === 0) return null
  return (
    <button
      type="button"
...
const AmenityCardInfo = ({ amenity, t }) => (
  <div className="amenity-info">
    ...
    <p className="amenity-days">
      {t('adminAmenities.days', { days: formatAvailableDays(amenity.availableDays || DEFAULT_AVAILABILITY.availableDays, t) })}
    </p>
...
const AmenityPhotoSection = ({ t, fileInputRef, photos, uploadingPhotos, uploadProgress, onPhotoUpload, onPhotoDelete }) => (
        ...
        {photos.map((photoUrl, index) => (
          <div key={index} className="photo-preview-item">
            ...
            <button
              ...
              onClick={() => onPhotoDelete(photoUrl, index)}
              disabled={uploadingPhotos}
...
const AmenityAvailabilitySection = ({ t, selectedAmenity, startHourRef, endHourRef, availableDays, onDayToggle }) => (
...
const AmenityForm = ({
  ...
  <form onSubmit={onSubmit}>
    ...
    <div className="form-group">
      ...
      <select name="type" className="form-field" defaultValue={selectedAmenity?.type || ''} onChange={onTypeChange} required>
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (pendingFiles.length > 0) {
        setUploadingPhotos(true)
        try {
          const uploadedPhotos = await uploadPendingPhotos(amenityId, pendingFiles, photos)
      ...
      <div className="container">
        ...
        <div className="amenities-grid">
          {amenities.map(amenity => (
            <AmenityCard
              ...
              amenity={amenity}
              t={t}
              onViewPhotos={setLightboxAmenity}
        ...
        >
          <AmenityForm
            ...
            onCancel={closeModal}
            capacityInputRef={capacityInputRef}
            startHourRef={startHourRef}
            endHourRef={endHourRef}
            fileInputRef={fileInputRef}
            photos={photos}
