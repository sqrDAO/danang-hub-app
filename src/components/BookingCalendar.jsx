...
const formatEventSpaceHours = (amenity, defaults, locale) => {
  ...
  const weekdaysLabel = getAvailableDaysLabel([1, 2, 3, 4, 5], locale)
  const weekendsLabel = getAvailableDaysLabel([0, 6], locale)

  return `${weekdayStart}:00 - ${endHour}:00 ${weekdaysLabel}, ${weekendStart.toString().padStart(2, '0')}:00 - ${endHour}:00 ${weekendsLabel}`
}
...
const getFormattedHoursInfo = (amenity, availability, locale) => {
  ...
  if (isEventSpace) {
    return formatEventSpaceHours(amenity, defaults, locale)
  }
